    ...
    >
      <el-form ref="form" :model="form" label-width="150px">
        <el-form-item label="Invoice ID">
          <el-input v-model="form.paymentId" :disabled="disableInput" />
        </el-form-item>
...
<script>
import { automobilePayment, getCustomerId, getFetchAxios, getUserRole, request, sendAxios } from '@/utils/global';
...
export default {
  ...
  methods: {
    fetchData() {
      getFetchAxios(this, automobilePayment);
    },
    addHandle() {
      console.log('Add');
      this.editbox = true;
    ...
    onSubmit() {
      ...
      if (this.disableInput === true) {
        sendAxios(this, request(automobilePayment.update) + this.editing, param)
      } else {
        sendAxios(this, request(automobilePayment.add), param)
      }
    },
    onCancel() {
      this.editbox = false;
    ...
    onDelete() {
      ...
      const param = new URLSearchParams();
      param.append('paymentId', this.form.paymentId);
      sendAxios(this, request(automobilePayment.delete) + this.editing, param);
    }
  }
};
</script>
